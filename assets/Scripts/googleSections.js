...
const genereteWebsitesElement = () => {
  ...
  for (let site of websitesData) {
    websitesWrapper.innerHTML += `<div class="favoriteWebsites__appBlock favoriteWebsites__appBlock--border flexDoubleAliningColumn">
    <a href="${site.siteAddress}" class="favoriteWebsites__appLink flexDoubleAliningColumn" target="_blank">
  ...
  for (let i = 0; i < 12 - websitesData.length; i++) {
    websitesWrapper.innerHTML += `<div class="favoriteWebsites__appBlock flexDoubleAliningColumn">
          <iconify-icon icon="mingcute:add-fill" class="favoriteWebsites__appIcon flexDoubleAliningRow"></iconify-icon>
      </div>`;
  }
};
